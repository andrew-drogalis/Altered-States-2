...
constexpr std::array<std::pair<std::string_view, int>, numStates> statesPopulation {{
...
constexpr std::array<double, numStates> peoplePerLetter()
{
    int i {};
    std::array<double, numStates> peoplePerLetterArr {};
    for (auto [state, population] : statesPopulation)
    {
        peoplePerLetterArr[i] = population / static_cast<double>(state.length());
        ++i;
    }
    return peoplePerLetterArr;
}


int main() { 
    auto people = peoplePerLetter();
    //
    // auto resultsMatrix;
    ...
    for (auto row : resultsMatrix) {
        for (auto col : row) {
            resultsMatrixStr.push_back(col);
        }
    }
    ...
    if (saveMatrix.good())
    {
        saveMatrix.write(resultsMatrixStr);
    }
    return 0;
}
